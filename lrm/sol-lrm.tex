...
\section{Expressions and Operators}
	...
	A typecast is the conversion of an expression from one type to another. SOL supports explicit casting of \textit{int} to \textit{float} \textit{float} to \textit{int}, and \textit{int}, \textit{float} and \textit{char} to \textit{string}. To cast a expression to a different type, place the desired type in parentheses in front of the expression. \\
